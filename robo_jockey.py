...
def party_score(df):
    # add party_score variable to a given dataframe

    # first load song library and add party_score variable to it
...
def generate_playlist(songlist_df,step_size):
    # compute party_score for songlist_df, then generate playlist according to user choice

    # load song library with party_score variable
    df = party_score(pd.read_csv("data/data_cleaned.csv"))

    # compute party_score for songlist_df
    songs_df = party_score(songlist_df)
    track_list = songs_df[["party_score"]]

    # determine playlist shape
    choice = input("Choose the vibe! Ebb-and-flow (1 / default) or The-sky-is-the-limit (2): ")

    if int(choice) == 2: # linear case

        # order songs_df by party_score (no fillers needed)
        return songs_df.sort_values(by = "party_score")


    else: # sinusoidal case
